...
public class CrptApi {
    ...
    /**
     * Конструктор для CrptApi.
     *
     * @param apiUrl      URL API
     * @param timeUnit    Единица времени для лимита запросов
     * @param requestLimit Лимит запросов за определенное время
     */
    public CrptApi(String apiUrl, TimeUnit timeUnit, int requestLimit) {
        ...
        // Периодически сбрасываем счетчик запросов
        scheduler.scheduleAtFixedRate(this::resetRequestCounter, 0, 1, timeUnit);
    }

    // Метод для сброса счетчика запросов
    private void resetRequestCounter() {
        semaphore.release(requestCounter.getAndSet(0));
    }
    ...
    public void createDocument(Object document, String signature) throws InterruptedException, IOException {
        semaphore.acquire(); // Ожидание освобождения семафора, если лимит превышен
...
// Класс для представления Description
class Description {
...
// Класс для представления Product
class Product {
...
// Класс для представления Document
class Document {
